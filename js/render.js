...
const renderWebGL = (glData, fullscreen, rotation) => {
    ...
    for (const attribute of programInfo.attributes) {
        if (attribute.location === null) {
            throw new Error(`${attribute.name} attribute location is null`);
        }
        if (attribute.buffer === null) {
            throw new Error(`${attribute.name} buffer is null`);
        }
    ...
    programInfo.uniforms[3].data = [0, viewSize / 2, -viewSize / 2];
    programInfo.uniforms[4].data = [0, 0, -viewSize / 2];
    programInfo.uniforms[6].data = [0, 1, 1];
    programInfo.uniforms[7].data = [0, 0, 1];
    programInfo.uniforms[8].data = [0.2, 0.2, 0.2];
    programInfo.uniforms[9].data = [0.6, 0.4, 0.2];
    programInfo.uniforms[10].data = [0.4, 0.5, 0.6];
    programInfo.uniforms[11].data = [1.0, 1.0, 1.0];
    programInfo.uniforms[12].data = 0.8;
    programInfo.uniforms[13].data = 0.95;
    ...
    for (let offset = -1; offset <= 1; offset += 1) {
        ...
        for (const uniform of programInfo.uniforms) {
            if (uniform.location === null) {
                throw new Error(`${uniform.name} uniform location is null`);
            }
            if (uniform.type === 'mat4') {
                if (!(uniform.data instanceof Array)) {
                    throw new Error(`${uniform.name} uniform data is not an array`);
                }
                ...
                if (typeof uniform.data !== 'number') {
                    throw new Error(`${uniform.name} uniform data is not a number`);
                }
                gl.uniform1f(uniform.location, uniform.data);
            }
            else if (uniform.type === 'sampler2D') {
                if (!(uniform.data instanceof WebGLTexture)) {
                    throw new Error(`${uniform.name} uniform data is not a WebGLTexture`);
                }

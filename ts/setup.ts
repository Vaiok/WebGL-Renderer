...
const setupWebGL = (canvas: HTMLCanvasElement): WebGLData => {
    ...
    for (const attribute of programInfo.attributes) {
        ...
        const methodName = `get${attribute.name[2].toUpperCase()}${attribute.name.slice(3)}s` as MethodName;
        gl.bufferData(gl.ARRAY_BUFFER, box[methodName](), gl.STATIC_DRAW);
        attribute.buffer = buffer;
    }
    for (const uniform of programInfo.uniforms) { uniform.location = gl.getUniformLocation(program, uniform.name); }
